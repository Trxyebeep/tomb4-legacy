...
void SkeletonControl(short item_number)
{
	...
	if (item->box_number == lara_item->box_number && item->mesh_bits & 0x200 ||
	...
	{
		if (info.angle >= 0x3000 || info.angle <= -12288)
		{
	...
	else
	{
		...
		larainfo.ahead = larainfo.angle > -0x4000 && larainfo.angle < 0x4000;	//originally uninitialized
